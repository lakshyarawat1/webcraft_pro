...
## 2024-07-23 - [Optimization: Avoid fetching heavy text content on getFunnel]
**Learning:** Similar to the previous finding on `getFunnels`, found a performance bottleneck where querying a single funnel using Prisma's `include: { FunnelPages: true }` in `getFunnel` loaded heavy JSON/text `content` column data for all pages into memory unnecessarily, leading to huge payload size and massive latency hits. This data is not used by the `FunnelSteps` or `FunnelSettings` components.
**Action:** Used nested `select` inside `include` (e.g. `include: { FunnelPages: { select: { id: true, name: true, ... } } }`) to pick only necessary fields instead of implicitly fetching everything when `content` isn't needed. Next time, always check if all fields in a large relationship are needed, specifically large string fields.

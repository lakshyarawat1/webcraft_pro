...
## 2024-07-20 - [Optimization: Avoid instantiating Intl objects in render loop]
**Learning:** Found a performance bottleneck where `Intl.NumberFormat` was instantiated inside the render loop of the `PipelineTicket` component. Instantiating `Intl` objects is extremely computationally expensive in JS engines (like V8) and creates significant lag, especially when rendering lists or collections where it gets recreated on every item render.
**Action:** Always declare `Intl` objects (e.g. `Intl.NumberFormat`, `Intl.DateTimeFormat`) outside of the component or function scope to reuse a single instance and prevent expensive recreating during rendering.

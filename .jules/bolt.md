...
## 2024-05-25 - [React Performance Pattern: Intl.NumberFormat instantiation in renders/loops]
**Learning:** Found multiple places where `new Intl.NumberFormat()` was being instantiated either inside a React component's render cycle (like in `PipelineLane.tsx` and `PipelineTicket.tsx`) or inside an array `.map()` loop (like in `contacts/page.tsx`). Instantiating `Intl` objects is computationally expensive in JS engines. Doing this in render cycles or loops degrades performance.
**Action:** Extract `Intl.NumberFormat` instantiation into a global/module-scoped constant and export a helper function (e.g., `formatCurrency` in `utils.ts`) so the same instance is reused application-wide, significantly saving compute time during renders and map iterations.

...
import {
  LaneDetail,
  PipelineDetailsWithLaneCardsTagsTickets,
  TicketAndTags,
} from "@/lib/types";
...
import { getUserByClerkId, updateTicketOrder } from "@/lib/queries";

type Props = {
  lanes: LaneDetail[];
  pipelineId: string;
  subAccountId: string;
  pipelineDetails: PipelineDetailsWithLaneCardsTagsTickets;
...
const PipelineView = ({
  ...
  const handleAddLane = () => {
    setOpen(
      <CustomModal
        title="Create a Lane"
        subHeading="Lanes allow you to group your tickets"
      >
    ...
    <DragDropContext onDragEnd={onDragEnd}>
      <div className="bg-white/60 dark:bg-background/60 rounded-xl p-4 use-automation-zoom-in">
        <div className="flex items-center justify-between">
          ...
          <Button onClick={handleAddLane}>Create a lane +</Button>
        </div>
        <Droppable
          droppableId="lanes"
          type="lane"
